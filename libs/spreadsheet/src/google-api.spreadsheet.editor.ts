...
export class GoogleApiSpreadsheetEditor {
  ...
  constructor(
    private readonly serviceCert: GoogleCert,
    private readonly isLock = false,
  ) {
    this.readSheet = this.readSheet.bind(this);
  }

  private oAuth2Client: GoogleAuth | OAuth2Client;

  static async create(
    serviceCert: GoogleCert,
    isLock = false,
  ): Promise<GoogleApiSpreadsheetEditor> {
    const instance = new GoogleApiSpreadsheetEditor(serviceCert, isLock);
    await instance.init();
    return instance;
  }

  async init(): Promise<void> {
    this.oAuth2Client = await this.serviceCert.init();
    this.isInitialized = this.serviceCert.isInitialize;
  }
  ...
  getAuthType(): GoogleAuth | OAuth2Client {
